...
def scaleImageToHeight(image, height=200):
    hpercent = (height / float(image.size[1]))
    wsize = int((float(image.size[0]) * float(hpercent)))
    return image.resize((wsize, height), Image.ANTIALIAS)

def save_image_sbs2(filename, *args):#content_image, style_image, mixed_image, filename):
    ...
    for arg in args:
        if isinstance(arg, str):
            ...
            image = arg
        scaled_image = scaleImageToHeight(image, height=height)
        npimg = np.asarray(scaled_image)
        if npimg.ndim != 3:
            print(" a gray scale image not implemented")
            return

        imagelist.append(np.asarray(scaled_image))

    imgs_comb = np.hstack(imagelist)
    print(imgs_comb.shape)
    # Write the image-file in jpeg-format.
    with open(filename, 'wb') as file:
        Image.fromarray(imgs_comb).save(file, 'jpeg')


def save_image_sbs(content_image, style_image, mixed_image, filename):
    # Ensure the pixel-values are between 0 and 255.
    imgs = [content_image, style_image, mixed_image]

    min_shape = sorted([(np.sum(i.size), i.size) for i in imgs])[0][1]
    #imgs_comb = np.hstack((np.asarray(i.resize(min_shape)) for i in imgs))

    #avoid ref error for np.resize or --> refcheck=False

    content_image.resize(min_shape, refcheck=False)
    style_image.resize(min_shape, refcheck=False)
    mixed_image.resize(min_shape, refcheck=False)
    ...
    scaled_images = [np.asarray(content_image), np.asarray(style_image), np.asarray(mixed_image)]

    imgs_comb = np.vstack(scaled_images)
    print(imgs_comb.shape)
    # Write the image-file in jpeg-format.
    with open(filename, 'wb') as file:
        Image.fromarray(imgs_comb).save(file, 'jpg')
...
def styletransferOnImage(inputpath):
    content_image = load_image(inputpath, max_size=300)


    style_folder = '/home/grusinator/git_ws/python/AAnder_geocatch/styles/'

    stylefilelist =[]
    for file in os.listdir(style_folder):
        if file.endswith(".jpg"):
            stylefilelist.append(file)


    style_filename = random.choice(stylefilelist)

    style_file_path = os.path.join(style_folder, style_filename)

    style_image = load_image(style_file_path, max_size=300)
